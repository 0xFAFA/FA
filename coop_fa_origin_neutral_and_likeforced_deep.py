...
def main():
    ...
    origin_neutral_multiply_factor = 2
    origin_likeforced_multiply_factor = 2
    ...
    cfg['origin_neutral_multiply_factor'] = 2
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neutral_and_likeforced_deep',multiply_factor_name+'','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        for train_idx in range(train_epoch):
            ...
            for i, (images, target) in enumerate(tqdm(train_loader)):
                ...
                logits,logits_local = model_stage2(images)
                loss_global = criterion(logits, target)

                
                # logits_local维度为[batch_size, 196, repeat_len]，target维度为[batch_size]
                # 现在只取第二维度（token数目）中，在第三维度的前1000个元素的gt索引对应位置值最大的token，
                # 即最终logits_local的维度为[batch_size, repeat_len]
                batch_size, token_len, repeat_len = logits_local.shape
                logits_local_temp = logits_local.view(batch_size, token_len, cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))

                

                # logits_local_forced 第二维度为token数，第三维度为classnum
                # 取第二维度（token数目）中，在第三维度的gt索引对应位置值最大的token

                logits_local_forced = logits_local_temp[:,:,0,:]

                # logits_local_forced 的shape为[batch_size,196,1000]
                # target 的shape为[batch_size]
                # 将target的维度扩展为[batch_size,196]，然后在第二维度（token数目）中，在第三维度的gt索引对应位置值最大的token

                target_temp = target.view(-1, 1).expand(-1, token_len)

                # 以target_temp为索引，取出logits_local_forced中对应位置的值，logits_local_forced_gt的shape为[batch_size,196]

                logits_local_forced_gt = torch.gather(logits_local_forced, dim=2, index=target_temp.unsqueeze(2)).squeeze(2)

                # 找到logits_local_forced_gt第二维度中值最大的token索引，logits_local_forced_token_index的shape为[batch_size]
                logits_local_forced_token_index = logits_local_forced_gt.argmax(dim=1)
                
                # 以logits_local_forced_token_index为索引，取出logits_local中对应token位置的值，logits_local_final的shape为[batch_size,repeat_len]


                logits_local_final = torch.gather(logits_local, dim=1, index=logits_local_forced_token_index.view(-1, 1, 1).expand(-1, -1, logits_local.shape[-1])).squeeze(1)

                
                loss_local = criterion(logits_local_final, target)




                # loss_local = criterion(logits_local.view(-1, logits_local.shape[-1]), target.repeat(1, logits_local.shape[1]).view(-1))
