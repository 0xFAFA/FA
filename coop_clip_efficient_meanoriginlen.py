...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        real_integer_part = int(cfg['multiply_factor']) - 1
        fractional_part = cfg['multiply_factor'] - integer_part
        
        # 整数部分的重复
        integer_repeated_tensor = self.text_features_classify_origin.repeat(real_integer_part, 1)

        # 间接使用expand，省内存
        # text_features_classify_origin_temp = self.text_features_classify_origin.unsqueeze(0)
        # integer_repeated_tensor = text_features_classify_origin_temp.expand(real_integer_part, text_features_classify_origin_temp.shape[1], text_features_classify_origin_temp.shape[2])
        # integer_repeated_tensor = integer_repeated_tensor.reshape(-1, integer_repeated_tensor.shape[-1])
        
        # 小数部分的处理
        if fractional_part > 0:
            # 计算需要额外添加的行数
            extra_rows = max(1, int(fractional_part * self.text_features_classify_origin.size(0)))
            # 截取原始张量的一部分来模拟小数部分的重复
            fractional_tensor = self.text_features_classify_origin[:extra_rows]
            # 如果有整数部分，就将两部分连接起来；如果没有，则只返回小数部分
            result_tensor = torch.cat((integer_repeated_tensor, fractional_tensor), dim=0) if real_integer_part > 0 else fractional_tensor
        else:
            result_tensor = integer_repeated_tensor
        
        self.text_features_classify_repeat = result_tensor
    ...
    def forward(self, image):
        ...
        if self.cfg['multiply_factor']-1 > 0:
            batch_size, token_len, repeat_len = logits_local.shape
            origin_len = len(self.classnames)

            # print(logits[0, :10])
            logits = logits.view(batch_size, self.cfg['multiply_factor'], origin_len )
            # print(logits[0, 0, :10])
            logits = logits.mean(dim=1)

            logits_local = logits_local.view(batch_size, token_len, self.cfg['multiply_factor'], origin_len )
            logits_local = logits_local.mean(dim=2)

        
        # print('logits_shape',logits.shape) #logits_shape torch.Size([160, 1000])
        # print('logits_local_shape',logits_local.shape) #logits_local_shape torch.Size([160, 196, 1000])
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_meanoriginlen_batchs160_ep50','lenx'+str(multiply_factor_name),'lr0002'  )  #  new_test   seed1   
    ...
    if is_train == 1:
        ...
        sys.stdout = Logger( os.path.join(cache_dir,f'log_test_ood_GL_meanoriginlen.txt') ,  stream=sys.stdout)
        print("\nRunning configs.")
        print(cfg, "\n")
