...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        logits = logits.view(batch_size, self.type_num + self.cfg['cur_stage']-1, self.classnum)
        logits_local = logits_local.view(batch_size, token_len, self.type_num + self.cfg['cur_stage']-1, self.classnum)

        # forced_logit
        # logits_final = logits[:,0,:]
        # logits_local_final = logits_local[:,:,0,:]

        logits_final = logits[:,0:self.cfg['cur_stage'] ,:]
        logits_final = logits_final.view(batch_size, -1)
        logits_local_final = logits_local[:,:,0:self.cfg['cur_stage'],:]
        ...
        if self.origin_neutral_repeat_num > 0:
            ...
            logits_origin_neutral = logits[:,self.cfg['cur_stage'],:]
            logits_local_origin_neutral = logits_local[:,:,self.cfg['cur_stage'],:]
        ...
        if self.origin_likeforced_repeat_num > 0:
            if self.origin_neutral_repeat_num > 0:
                ...
                logits_origin_likeforced = logits[:,self.cfg['cur_stage']+1,:]
                logits_local_origin_likeforced = logits_local[:,:,self.cfg['cur_stage']+1,:]
            else:
                # logits_origin_likeforced = logits[:,1,:]
                # logits_local_origin_likeforced = logits_local[:,:,1,:]
                logits_origin_likeforced = logits[:,self.cfg['cur_stage'],:]
                logits_local_origin_likeforced = logits_local[:,:,self.cfg['cur_stage'],:]
...
def main():
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = f"log_test_ood_common_stage{cfg['cur_stage']}.txt"
        else:
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                logits_id_temp = logits_id.view(batch_size, cfg['cur_stage']-1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))
                ...
                logits_id_local_temp = logits_id_local.view(batch_size, token_len, cfg['cur_stage']-1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))
