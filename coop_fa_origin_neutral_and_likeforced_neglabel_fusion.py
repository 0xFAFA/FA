...
def main():
    ...
    neg_label_list = neg_label_list[ 500 : (500+ (cfg['origin_neglabel_len'] * 1000)) ]

    # 向neg_label_list里拼接2000个a photo of a thing that we can see in nature.
    neg_label_list = neg_label_list + ['a photo of a thing that we can see in nature.'] * 2000
    ...
    model_name = 'fa_myfusion'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neglabel',multiply_factor_name+'_'+str(cfg['origin_neglabel_len'])+'_2_slice1-5','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
