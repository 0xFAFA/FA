...
def main():
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neutral_and_likeforced_mutiloss2',multiply_factor_name+'','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        for train_idx in range(train_epoch):
            ...
            for i, (images, target) in enumerate(tqdm(train_loader)):
                ...
                batch_size, repeat_len = logits.shape
                logits_temp = logits.view(batch_size, 1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor'] , len(cfg['classnames']))

                logits_forced = logits_temp[:,[0],:]
                logits_origin_neutral = logits_temp[:,1:(1+cfg['origin_neutral_multiply_factor']),:]
                logits_origin_likeforced = logits_temp[:,(1+cfg['origin_neutral_multiply_factor']):,:]

                # logits1 = torch.cat([logits_forced, logits_origin_neutral], dim=1)
                # logits2 = torch.cat([logits_forced, logits_origin_likeforced], dim=1)

                # logits1 = logits1.view(batch_size, -1)
                # logits2 = logits2.view(batch_size, -1)


                logits1 = logits
                logits2 = logits_forced.view(batch_size, -1)
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = 'log_test_ood_common2-0.txt'
        else:
        ...
        with torch.no_grad():
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    smax_global = F.softmax(logits_ood/T, dim=-1) #[:, :len(cfg['classnames'])]
                    smax_global = smax_global.cpu().numpy()
                    mcm_global_score = np.max(smax_global, axis=1)

                    smax_local = F.softmax(logits_ood_local/T, dim=-1) #[:, :, :len(cfg['classnames'])]
                    smax_local = smax_local.cpu().numpy()
                    mcm_local_score = np.max(smax_local, axis=(1, 2))
