...
def main():
   ...
   # neutral=0 : likeforced=2 相当于之前lenx3
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'with_neutral_stepwise','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = f"log_test_ood_common_stage{cfg['cur_stage']}-{cfg['origin_neutral_multiply_factor']}-{cfg['origin_likeforced_multiply_factor']}.txt"
        else:
