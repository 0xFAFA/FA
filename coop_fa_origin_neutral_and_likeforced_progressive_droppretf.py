...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        self.type_num = 3
        self.text_features_forced_previous = None
    ...
    def forward(self, image):
        ...
        # concat
        if self.cfg['cur_stage_droppretf'] > 1:
            text_features_concat = torch.cat((text_features_forced, self.text_features_forced_previous, self.text_features_origin_neutral, self.text_features_origin_likeforced), dim=0)
        else:
        ...
        logits = logits.view(batch_size, self.type_num + self.cfg['cur_stage_droppretf']-1, self.classnum)
        logits_local = logits_local.view(batch_size, token_len, self.type_num + self.cfg['cur_stage_droppretf']-1, self.classnum)

        # forced_logit
        # logits_final = logits[:,0,:]
        # logits_local_final = logits_local[:,:,0,:]

        logits_final = logits[:,0:self.cfg['cur_stage_droppretf'] ,:]
        logits_final = logits_final.view(batch_size, -1)
        logits_local_final = logits_local[:,:,0:self.cfg['cur_stage_droppretf'],:]
        logits_local_final = logits_local_final.view(batch_size, token_len, -1)


        if self.origin_neutral_repeat_num > 0:
            # logits_origin_neutral = logits[:,1,:]
            # logits_local_origin_neutral = logits_local[:,:,1,:]
            logits_origin_neutral = logits[:,self.cfg['cur_stage_droppretf'],:]
            logits_local_origin_neutral = logits_local[:,:,self.cfg['cur_stage_droppretf'],:]
        ...
        if self.origin_likeforced_repeat_num > 0:
            if self.origin_neutral_repeat_num > 0:
                ...
                logits_origin_likeforced = logits[:,self.cfg['cur_stage_droppretf']+1,:]
                logits_local_origin_likeforced = logits_local[:,:,self.cfg['cur_stage_droppretf']+1,:]
            else:
                # logits_origin_likeforced = logits[:,1,:]
                # logits_local_origin_likeforced = logits_local[:,:,1,:]
                logits_origin_likeforced = logits[:,self.cfg['cur_stage_droppretf'],:]
                logits_local_origin_likeforced = logits_local[:,:,self.cfg['cur_stage_droppretf'],:]
...
def main():
    ...
    # cfg['origin_likeforced_multiply_factor'] = 2
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neutral_and_likeforced_progressive',multiply_factor_name+'','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        sys.stdout = Logger( os.path.join(cache_dir, f"log_train_stage{cfg['cur_stage']}.txt") ,  stream=sys.stdout)
        print("\nRunning configs.")
        print(cfg, "\n")
        ...
        print(f"current stage: {cfg['cur_stage']}")
        ...
        # Model
        model = CustomCLIP(cfg, cfg['classnames'], clip_model ,cfg['template'],cfg['template_origin_neutral'], False, cfg['csc_ctx_len'], cfg['csc'])
        model = torch.nn.DataParallel(model).to(device)

        # load previous text_features_forced
        if cfg['cur_stage'] > 1:
            previous_stage = cfg['cur_stage'] - 1
            text_features_forced = torch.load(os.path.join(cfg['cache_dir'], 'text_features', f'text_features_forced_stage{previous_stage}.pth'))
            model.module.text_features_forced_previous = text_features_forced
            print(f"text_features_forced_stage{previous_stage}.pth shape: {text_features_forced.shape}")
        ...
        for train_idx in range(train_epoch):
            ...
            #         # save model
            #         torch.save(model.state_dict(), os.path.join(cfg['cache_dir'], 'model_init.pth'))
            ...
            if  train_idx == train_epoch - 1 :   
                ...
                with torch.no_grad():
                    ...
                    # save model
                    torch.save(model.state_dict(), os.path.join(cfg['cache_dir'], f"model_stage{cfg['cur_stage']}.pth"))

                    # save forced text features
                    os.makedirs( os.path.join(cache_dir,'text_features' ) , exist_ok=True)
                    ...
                    text_features_forced = text_encoder(prompts, tokenized_prompts).type(model.module.dtype)

                    torch.save(text_features_forced, os.path.join(cfg['cache_dir'], 'text_features', f"text_features_forced_stage{cfg['cur_stage']}.pth"))
                    ...
                    for images, labels in tqdm(test_loader):
                        ...
                        outputs = outputs / 100.0
                        batch_size, repeat_len = outputs.shape
                        outputs_temp = outputs.view(batch_size, cfg['cur_stage']-1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))
                        just_origin_outputs = outputs_temp[:,0,:]
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = f"log_test_ood_common_stage{cfg['cur_stage']}_droppretf.txt"
        else:
        ...
        # load model
        model.load_state_dict(torch.load(os.path.join(cfg['cache_dir'], f"model_stage{cfg['cur_stage']}.pth")))

        # load previous text_features_forced
        if cfg['cur_stage_droppretf'] > 1:
            previous_stage = cfg['cur_stage_droppretf'] - 1
            text_features_forced = torch.load(os.path.join(cfg['cache_dir'], 'text_features', f"text_features_forced_stage{previous_stage}.pth"))
            model.module.text_features_forced_previous = text_features_forced
            print(f"text_features_forced_stage{previous_stage}.pth shape: {text_features_forced.shape}")
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                logits_id_temp = logits_id.view(batch_size, cfg['cur_stage_droppretf']-1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))
                ...
                logits_id_local_temp = logits_id_local.view(batch_size, token_len, cfg['cur_stage_droppretf']-1+cfg['origin_neutral_multiply_factor']+cfg['origin_likeforced_multiply_factor']+1 , len(cfg['classnames']))
