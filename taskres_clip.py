...
class TextResLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_feature_residuals= nn.Parameter(torch.zeros( len(classnames), cfg['embed_dim']))


    def forward(self, text_features):
        text_feature_residuals = self.text_feature_residuals
        return text_features  + self.alpha * text_feature_residuals #   t + a * x
...
def main():
    ...
    model_name = 'taskres'
    cache_dir = os.path.join('./mycaches', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name,f'imgnet classname lenx{multiply_factor} lr00002'  )  #  new_test   seed1
