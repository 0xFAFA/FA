...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, cache_keys, cache_values):
        super().__init__()
        ...
        self.value_weights = nn.Parameter(torch.ones([self.classnum*cfg['shots'], 1]).half().cuda())
    ...
    def forward(self, image):
        ...
        keys_res = self.residual_learner.unsqueeze(1).repeat(1, self.cfg['shots'], 1).reshape(-1, self.cfg['embed_dim'])
        new_cache_values = self.value_weights * self.cache_values
        ...
        logits_cache_forced = affinity @ new_cache_values
        ...
        logits_cache_local_forced = affinity_local @ new_cache_values
        ...
        if self.origin_neutral_repeat_num > 0:
            ...
            logits_cache_origin = affinity_origin @ new_cache_values
            ...
            logits_cache_local_origin = affinity_local_origin @ new_cache_values
...
def extract_cache_image_feature(cfg, clip_model, train_data_loader):
    cache_keys = []
    cache_values = []
    with torch.no_grad():
...
def main():
    ...
    model_name = 'fa_taskres_cache2'
    lrname = str(cfg['lr']).replace('.','')
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'newformula_valueweight','neutral_len'+str(origin_neutral_multiply_factor),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        if is_extract_feature == 1:
            train_loader_extract = build_data_loader(data_source=few_shot_dataset_stage2.train_x, batch_size=train_batch_size, tfm=train_transform_aug, is_train=True, shuffle=False)
            extract_cache_image_feature(cfg, clip_model, train_loader_extract) # save cache keys and values
        ...
        for name, param in model_stage2.named_parameters():
            if  "residual_learner" in name  or "value_weights" in name: # 
