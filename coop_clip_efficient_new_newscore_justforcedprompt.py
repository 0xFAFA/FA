...
def main():
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = 'log_test_ood_common_newscore_justforcedprompt_check.txt'
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                smax_global = F.softmax(just_origin_logits_id/T, dim=-1).cpu().numpy() # mcm无论分子分母都只用forced prompt
                # print('smax_global.shape',smax_global.shape) # [B,len(class)*multiply_factor]

                # print(smax_global[1,0:3])
                # print(smax_global[1,100:103])
                # print(smax_global[1,200:203])


                mcm_global_score = np.max(smax_global, axis=1)
                # print(mcm_global_score.shape) # [B,]

                smax_local = F.softmax(just_origin_logits_id_local/T, dim=-1).cpu().numpy()
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    # logits_ood_temp = logits_ood.view(batch_size, cfg['multiply_factor'], len(cfg['classnames']))
                    # just_origin_logits_ood = logits_ood_temp[:,0,:]
                    just_origin_logits_ood = logits_ood[:, :len(cfg['classnames'])]

                    # logits_ood_local_temp = logits_ood_local.view(batch_size, token_len, cfg['multiply_factor'], len(cfg['classnames']))
                    # just_origin_logits_ood_local = logits_ood_local_temp[:,:,0,:]
                    just_origin_logits_ood_local = logits_ood_local[:, :, :len(cfg['classnames'])]




                    smax_global = F.softmax(just_origin_logits_ood/T, dim=-1).cpu().numpy()
                    # print('smax_global.shape',smax_global.shape) # [B,len(class)*multiply_factor]
                    ...
                    smax_local = F.softmax(just_origin_logits_ood_local/T, dim=-1).cpu().numpy()
                    mcm_local_score = np.max(smax_local, axis=(1, 2))
