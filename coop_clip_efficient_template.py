...
def get_origin_text_features_template(cfg, text_encoder, clip_model):
    device = next(text_encoder.parameters()).device
    if clip_model.dtype == torch.float16:
        text_encoder = text_encoder.cuda()
    
    complete_template = cfg['complete_template']
    classnames = cfg['classnames']
    
    with torch.no_grad():
        text_embeddings = [] # [num_classes, template_num, transformer.width]
        for text in classnames:
            tokens = clip.tokenize([template.format(text) for template in complete_template]).to(device)  # tokenized prompts are indices
            embeddings = clip_model.token_embedding(tokens).type(clip_model.dtype).to(device)

            # print(tokens.shape,'tokens.shape') # torch.Size([n, 77]) tokens.shape, n个template
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        self.text_features_classify_origin = get_origin_text_features_template(cfg, self.text_encoder, clip_model)
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_template_batchs500_ep100','lenx'+str(multiply_factor_name),'lr0002'  )  #  new_test   seed1   
    ...
    if is_train == 1:
        ...
        cfg['complete_template'] = few_shot_dataset_stage2.template #
