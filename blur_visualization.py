...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model, cache_keys_forced, cache_values_forced, cache_keys_original, cache_values_original):
        ...
        self.residual_learner_visual = nn.Parameter(torch.zeros( self.classnum , cfg['embed_dim']).half().cuda() )
    ...
    def forward(self, image):
        ...
        keys_res = self.residual_learner_visual.unsqueeze(1).repeat(1, self.cfg['shots'], 1).reshape(-1, self.cfg['embed_dim'])
...
def main():
    ...
    model_name = 'blur_visualization'
    lrname = str(cfg['lr']).replace('.','')
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'trainMtrm_fcachenotrm_ocacheblurtrm','neutral_len'+str(origin_neutral_multiply_factor),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    transform_aug_blur = transforms.Compose([
        ...
        transforms.GaussianBlur(kernel_size=15, sigma=(3.0, 7.0)),
    ...
    cache_transform_forced = train_transform_no_aug
    ...
    if is_train == 1:
        ...
        if is_extract_feature == 1:
            ...
            train_loader_extract_original = build_data_loader(data_source=few_shot_dataset_stage2.train_x, batch_size=50, tfm=cache_transform_original, is_train=True, shuffle=False)

            
            # 遍历train_loader_extract_original
            for tensor_image, _ in train_loader_extract_original:
                print('tensor_image shape:', tensor_image.shape)  
                for i in range(tensor_image.shape[0]):
                    tensor_image[i] = tensor_image[i].float()
                    tensor_image_t = tensor_image[i] 
                    print('tensor_image shape:', tensor_image.shape)
                    image = tensor_image_t.permute(1, 2, 0).cpu().numpy()  # CHW -> HWC

                    plt.imshow(image)
                    plt.axis('off')
                    plt.savefig(cache_dir+'/'+f'image_{i}.png', bbox_inches='tight', pad_inches=0)
                    plt.close()  # 关闭图像资源
