...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        # result_tensor = self.text_features_classify_origin.repeat(real_integer_part, 1)


        result_tensor_multi = []
        for index in range(real_integer_part):
            cur_template = cfg['complete_template'][index]
            cur_prompt_learner = PromptLearner(classnames, clip_model, CTX_INIT=cur_template, N_CTX=n_ctx, CSC = csc, cfg = cfg)
            cur_text_features_classify = get_origin_text_features(cur_prompt_learner, self.text_encoder, clip_model)
            result_tensor_multi.append(cur_text_features_classify)

        result_tensor = torch.cat(result_tensor_multi, dim=0)
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_multitrain_repeat_batchs100_ep50','lenx'+str(multiply_factor_name),'lr0002'  )  #  new_test   seed1   
