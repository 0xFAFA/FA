...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        # # ------其他学习
        # self.text_feature_residuals = nn.Parameter(torch.zeros( len(classnames) * cfg['multiply_factor'], cfg['embed_dim']))
        
        # ------其他不学习
        self.text_feature_residuals_origin = nn.Parameter(torch.zeros( len(classnames), cfg['embed_dim']))
        self.text_feature_residuals = self.text_feature_residuals_origin
        if cfg['multiply_factor'] > 1:
            self.text_feature_residuals_multiply = torch.zeros( int(len(classnames) * (cfg['multiply_factor']-1) ), cfg['embed_dim']).to(self.device)
    ...
    def forward(self, image):
        ...
        text_features_classify_final = text_features_classify_final + self.alpha * text_feature_residuals
        text_features_classify_final = text_features_classify_final.type(self.dtype)
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_taskrespart_batchs100_ep30','lenx'+str(multiply_factor_name),'lr0002'  )  #  new_test   seed1   
    ...
    if is_train == 1:
        ...
        for name, param in model_stage2.named_parameters():
            if  "prompt_learner" in name or "text_feature_residuals" in name: #  
