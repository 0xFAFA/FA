...
class PromptLearner(nn.Module):
    def __init__(
        ...
        print(
            self.ctx.shape
        )  # 当为"a photo of a"时为[4, 512]，csc为True时为[1000, 4, 512]
        ...
        tokenized_prompts = torch.cat([clip.tokenize(p) for p in prompts]).to(
            clip_model.positional_embedding.device
        )
...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts):
        x = prompts.type(self.dtype) + self.positional_embedding.to(
            dtype=self.dtype, device=prompts.device
        )
...
def get_origin_neglabel_text_features(
    origin_neglabel_list, classnames, text_encoder, clip_model
):
    device = next(text_encoder.parameters()).device
    ...
    with torch.no_grad():
        ...
        origin_text_features_list = []

        for i in range(0, len(prompts), 200):
            prompts_batch = prompts[i : i + 200]
            prompts_batch = [clip.tokenize(p) for p in prompts_batch]
            prompts_batch = torch.cat(prompts_batch).to(
                clip_model.positional_embedding.device
            )
            embedding_prompts = clip_model.token_embedding(prompts_batch).type(
                clip_model.dtype
            )

            origin_text_features = text_encoder(
                embedding_prompts.cuda(), prompts_batch.cuda()
            )
            origin_text_features_list.append(origin_text_features)
        ...
        origin_text_features = torch.cat(origin_text_features_list, dim=0)
...
class CustomCLIP(nn.Module):
    def __init__(
        ...
        self.prompt_learner = PromptLearner(
            classnames, clip_model, CTX_INIT=CTX_INIT, N_CTX=n_ctx, CSC=csc, cfg=cfg
        )
        ...
        self.text_features_origin_label = get_origin_neglabel_text_features(
            cfg["origin_neglabel_list"], classnames, self.text_encoder, clip_model
        )

        self.text_features_classify_origin = get_origin_text_features(
            self.prompt_learner, self.text_encoder, clip_model
        )
        ...
        print(
            f"self.text_features_classify_repeat.shape: {self.text_features_classify_repeat.shape}"
        )

    def forward(self, image):
        if self.single:
            image = self.CLIP_Transforms(image)
        ...
        prompts = self.prompt_learner().to(
            image.device
        )  # [len(classnames), ctx_sum, transformer.width] 如[1000, 77, 512]
        tokenized_prompts = self.tokenized_prompts.to(
            image.device
        )  # [len(classnames), 77]
        ...
        # confuses
        text_features_classify_final = torch.cat(
            (
                text_features,
                self.text_features_classify_repeat,
                self.text_features_origin_label,
            ),
            dim=0,
        )
        ...
        text_features_classify_final = (
            text_features_classify_final
            / text_features_classify_final.norm(dim=-1, keepdim=True)
        )
        ...
        logits = (
            logit_scale
            * image_features
            @ text_features_classify_final.transpose(-1, -2)
        )
        logits_local = (
            logit_scale * local_image_features @ text_features_classify_final.T
        )
        ...
        logits_neglabel = logits[
            :, -(self.cfg["origin_neglabel_len"]) :
        ]  # * self.classnum
        logits_local_neglabel = logits_local[
            :, :, -(self.cfg["origin_neglabel_len"]) :
        ]  # * self.classnum

        logits = logits[:, : -(self.cfg["origin_neglabel_len"])]  # * self.classnum
        ...
        if self.repeat_sum_num - 1 > 0:
            ...
            logits_repeat_context = logits_repeat_template.repeat(
                1, self.repeat_sum_num - 1
            )
            ...
            logits_local_repeat_template = logits_local.view(
                batch_size, token_len, 2, self.classnum
            )[:, :, 1, :]
            # print(f"logits_local_repeat_template.shape: {logits_local_repeat_template.shape}")
            logits_local_repeat_context = logits_local_repeat_template.repeat(
                1, 1, self.repeat_sum_num - 1
            )
        ...
        logits = torch.cat((logits, logits_neglabel), dim=1)
        logits_local = torch.cat((logits_local, logits_local_neglabel), dim=2)
        # print(f"logits.shape: {logits.shape}")
        # print(f"logits_local.shape: {logits_local.shape}")

        return logits, logits_local

def grouping(pos, neg, num, ngroup=100, random_permute=True):
    group = ngroup
    ...
    if random_permute:
        ...
        idx = torch.randperm(neg.shape[1], device="cuda")
        neg = neg.T
    ...
    for i in range(group):
        full_sim = torch.cat([pos, negs[:, i, :]], dim=-1) / 1.0
...
def get_arguments():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/id_dataset.yaml",
        help="settings in yaml format",
    )
...
def main():
    ...
    model_path = os.path.join(
        "/home/enroll2024/xinhua/.cache/clip/",
        model_file_name_dict[cfg["backbone"]] + ".pt",
    )
    ...
    cfg["origin_neglabel_len"] = 8000
    cfg["origin_neglabel_group"] = 80
    ...
    # neg_label_txt = "./selected_neg_labels_in1k_10k.txt"
    neg_label_txt = "./selected_neg_labels.txt"  # csp
    # neg_label_txt = "./selected_neg_labels_fusion.txt"  # fusion
    neg_label_list = []
    ...
    neg_label_list = neg_label_list[: (cfg["origin_neglabel_len"])]
    ...
    cache_dir = os.path.join(
        "./mycaches_id",
        cfg["id_dataset"],
        model_file_name_dict[cfg["backbone"]],
        str(cfg["shots"]) + "shots",
        model_name
        + "_efficient_batchs"
        + str(cfg["fine_tune_batch_size"])
        + "_ep"
        + str(cfg["fine_tune_train_epoch"])
        + "",
        "lenx" + str(multiply_factor_name),
        "lr" + lrname,
        "seed" + str(cfg["seed"]),
    )  #
    ...
    # train_transform_aug_coop = transforms.Compose([
    ...
    if is_train == 1:
        ...
        # stage2 dataset
        few_shot_dataset_stage2 = build_dataset(
            cfg["id_dataset"], cfg["root_path"], cfg["shots"]
        )
        batch_size_stage2 = cfg["fine_tune_batch_size"]

        val_loader = build_data_loader(
            data_source=few_shot_dataset_stage2.val,
        ...
        test_loader = build_data_loader(
            data_source=few_shot_dataset_stage2.test,
            batch_size=batch_size_stage2,
            is_train=False,
            tfm=train_transform_no_aug,
            shuffle=False,
        ...
        train_loader = build_data_loader(
            data_source=few_shot_dataset_stage2.train_x,
            batch_size=batch_size_stage2,
            tfm=train_transform_aug,
            is_train=True,
            shuffle=True,
        )
        ...
        # Model_stage2
        model_stage2 = CustomCLIP(
            cfg, cfg["classnames"], clip_model, cfg["template"], False, 16, cfg["csc"]
        )
        ...
        for train_idx in range(train_epoch):
            ...
            print(
                "LR: {:.6f}, train_acc: {:.4f} ({:}/{:}), Loss: {:.4f}".format(
                    current_lr,
            ...
            # test
            if (
                train_idx == train_epoch - 1
            ):  #  (train_idx + 1) % 100 == 0 or  train_idx == 0  or # 待修改为取最后一个epoch
                model_stage2.eval()
                with torch.no_grad():
                    correct_samples, all_samples = 0, 0
                    topk = 1
                    ...
                    for images, labels in tqdm(test_loader):
                        ...
                        outputs_temp = outputs.view(
                            batch_size, cfg["multiply_factor"], len(cfg["classnames"])
                        )
                    ...
                    print(
                        f"test Epoch [{train_idx + 1}/{train_epoch}], Test accuracy of the model on the test images: {100 * test_acc:.2f}%"
                    )
                    ...
                    # save model
                    torch.save(
                        model_stage2.state_dict(),
                        os.path.join(cfg["cache_dir"], "model.pth"),
                    )
        ...
        if cfg["ood_dataset"] == "challenging":
            ...
            test_file_name = (
                f"log_test_ood_common_fuxian_csp{cfg['origin_neglabel_len']}_t1.txt"
            )
        else:
        ...
        model_stage2.load_state_dict(
            torch.load(os.path.join(cfg["cache_dir"], "model.pth"))
        )
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                # logits_id_temp = logits_id.view(batch_size, cfg['multiply_factor']+cfg['origin_neglabel_len'] , len(cfg['classnames']))
                ...
                # logits_id_local_temp = logits_id_local.view(batch_size, token_len, cfg['multiply_factor']+cfg['origin_neglabel_len'], len(cfg['classnames']))
                # just_origin_logits_id_local = logits_id_local_temp[:,:,0,:]
                just_origin_logits_id_local = logits_id_local[
                    :, :, : len(cfg["classnames"])
                ]
                # ----
                ...
                # neg label 分组
                # t1
                logits_pos = logits_id[:, : (cfg["multiply_factor"] * len(cfg["classnames"])) ]  # 
                logits_neg = logits_id[:, (cfg["multiply_factor"] * len(cfg["classnames"])) :] #

                # t2
                # logits_pos = logits_id[:, : ( len(cfg["classnames"])) ]  # cfg["multiply_factor"] *
                # logits_neg = logits_id[:, ( len(cfg["classnames"])) :] #cfg["multiply_factor"] *
                ...
                my_score = grouping(
                    logits_pos, logits_neg, None, cfg["origin_neglabel_group"], True
                )  
                my_score = my_score.cpu().numpy()




                id_conf = np.concatenate((id_conf, mcm_global_score))
                id_conf_gl = np.concatenate(
                    (id_conf_gl, mcm_global_score + mcm_local_score)
                )
                ...
                correct = pred.eq(labels.view(1, -1).expand_as(pred))
                acc_num = float(
                    correct[:topk].reshape(-1).float().sum(0, keepdim=True).cpu().numpy()
                )
                correct_samples += acc_num
                all_samples += labels.shape[0]

                logits_id_local_gl = (
                    just_origin_logits_id_local.max(dim=1)[0] + just_origin_logits_id
                )
                pred_gl = logits_id_local_gl.topk(topk, 1, True, True)[1].t()
                correct_gl = pred_gl.eq(labels.view(1, -1).expand_as(pred_gl))
                acc_num_gl = float(
                    correct_gl[:topk].reshape(-1).float().sum(0, keepdim=True).cpu().numpy()
                )
                correct_samples_gl += acc_num_gl
                all_samples_gl += labels.shape[0]
            ...
            print(
                f"Test accuracy of the model on the test images: {100 * test_acc:.2f}%"
            )
            print(
                f"Test gl accuracy of the model on the test images: {100 * test_acc_gl:.2f}%"
            )
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                if cur_ood_dataset == "ood":
                    ood_dataset = datasets.ImageFolder(
                        root=os.path.join(cfg["ood_dataset_path"], cur_ood_dataset),
                        transform=train_transform_no_aug,
                    )
                elif cur_ood_dataset in [
                    "iNaturalist",
                    "SUN",
                    "Places",
                    "OpenImage_O",
                    ...
                    ood_dataset = datasets.ImageFolder(
                        root=os.path.join(
                            cfg["ood_dataset_path"], cur_ood_dataset, "images"
                        ),
                        transform=train_transform_no_aug,
                    )
                ...
                #     ood_dataset = datasets.ImageFolder(root=os.path.join(cfg['ood_dataset_path'], cur_ood_dataset, ), transform=train_transform_no_aug)
                    ...
                    ood_dataset = datasets.ImageFolder(
                        root=os.path.join(
                            cfg["ood_dataset_path"], cur_ood_dataset, "test"
                        ),
                        transform=train_transform_no_aug,
                    )
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    # neg label 分组
                    # t1
                    logits_pos = logits_ood[:, : (cfg["multiply_factor"] * len(cfg["classnames"])) ]  # 
                    logits_neg = logits_ood[:, (cfg["multiply_factor"] * len(cfg["classnames"])) :] #

                    # t2
                    # logits_pos = logits_ood[:, : ( len(cfg["classnames"])) ]  # cfg["multiply_factor"] *
                    # logits_neg = logits_ood[:, ( len(cfg["classnames"])) :] #cfg["multiply_factor"] *
                    ...
                    ood_conf_gl = np.concatenate(
                        (ood_conf_gl, mcm_global_score + mcm_local_score)
                    )
                ...
                avg_fpr_gl += fpr_gl
                print(
                    f"AUROC_glmcm: {auroc_gl:.4f}, AUPR_glmcm: {aupr_gl:.4f}, FPR(0.95)_glmcm: {fpr_gl:.4f}"
                )
                ...
                avg_fpr_my += fpr_my
                print(
                    f"AUROC_my: {auroc_my:.4f}, AUPR_my: {aupr_my:.4f}, FPR(0.95)_my: {fpr_my:.4f}"
                )
            ...
            print(
                f"Average AUROC: {avg_auroc:.4f}, Average AUPR: {avg_aupr:.4f}, Average FPR(0.95): {avg_fpr:.4f}"
            )
            ...
            print(
                f"Average AUROC_glmcm: {avg_auroc_gl:.4f}, Average AUPR_glmcm: {avg_aupr_gl:.4f}, Average FPR(0.95)_glmcm: {avg_fpr_gl:.4f}"
            )
            ...
            print(
                f"Average AUROC_my: {avg_auroc_my:.4f}, Average AUPR_my: {avg_aupr_my:.4f}, Average FPR(0.95)_my: {avg_fpr_my:.4f}"
            )
