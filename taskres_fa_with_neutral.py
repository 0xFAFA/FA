...
class TextResLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_feature_residuals= nn.Parameter(torch.zeros( self.origin_classnum , cfg['embed_dim']))
        # ------
...
class CustomClipTextEncoder(nn.Module):
    ...
    def forward(self):
        self.text_features_origin = self.text_features_origin.type(self.dtype).to(self.cfg['device'])

        final_text_features = self.text_residual_learner(self.text_features_origin)
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # concat
        text_features_concat = torch.cat((text_features, self.text_features_origin_neutral,), dim=0)
...
def main():
    ...
    model_name = 'fa_taskres'
    lrname = str(cfg['lr']).replace('.','')
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'with_neutral_multemplate_check','neutral_len'+str(origin_neutral_multiply_factor),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
