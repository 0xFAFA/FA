...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # confuses
        text_features_classify_final = text_features #torch.cat((text_features, self.text_features_classify_repeat), dim=0)
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient','lenx'+str(multiply_factor_name),'lr0002'  )  #  new_test   seed1   
    ...
    if is_train == 1:
        ...
        # Model_stage2
        model_stage2 = CustomCLIP(cfg, cfg['classnames'], clip_model ,cfg['template'], False, 16)
        ...
        sys.stdout = Logger( os.path.join(cache_dir,f'log_test_ood_GL_return_origin.txt') ,  stream=sys.stdout)
        print("\nRunning configs.")
        print(cfg, "\n")
