...
def get_origin_text_features(origin_prompt, classnames, text_encoder, clip_model):
    device = next(text_encoder.parameters()).device
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
                 ...
                 CTX_INIT_ORIGIN = "a photo of a thing that we can see in nature.", 
                 single = False, 
                 n_ctx = 16,
                 csc = False):
        super().__init__()
        ...
        # self.text_features_classify_origin = get_origin_text_features(self.prompt_learner, self.text_encoder, clip_model)
        self.text_features_classify_origin = get_origin_text_features(CTX_INIT_ORIGIN, classnames, self.text_encoder, clip_model)
...
def main():
    ...
    cfg['template_origin'] = 'a photo of a thing that we can see in nature.'  # 
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neutral','lenx'+str(multiply_factor_name),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        # Model_stage2
        model_stage2 = CustomCLIP(cfg, cfg['classnames'], clip_model ,cfg['template'],cfg['template_origin'], False, cfg['csc_ctx_len'], cfg['csc'])
        ...
        for train_idx in range(train_epoch):
            ...
            # test
            if  train_idx == 0 :   #  (train_idx + 1) % 100 == 0 or    or # 待修改为取最后一个epoch
                model_stage2.eval()
                with torch.no_grad():
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = 'log_test_ood_common_likepaper_init_check2.txt'
        else:
            test_file_name = f'log_test_ood_{cfg["ood_dataset"]}.txt'
        sys.stdout = Logger( os.path.join(cache_dir, 'newscore', test_file_name) ,  stream=sys.stdout)
        print("\nRunning configs.")
        print(cfg, "\n")
        ...
        # load model
        model_stage2.load_state_dict(torch.load(os.path.join(cfg['cache_dir'], 'model_init.pth')))
        ...
        with torch.no_grad():
            ...
            id_conf_my_mcm = np.array([])
            id_conf_my_glmcm = np.array([])
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                smax_global_tensor = F.softmax(logits_id/T, dim=-1)
                # smax_global_tensor_temp = smax_global_tensor.view(batch_size, cfg['multiply_factor'], len(cfg['classnames']))
                # my_smax_global_tensor = smax_global_tensor_temp[:,0,:]

                smax_global = F.softmax(logits_id/T, dim=-1).cpu().numpy()
                my_smax_global_tensor = smax_global_tensor[:, :len(cfg['classnames'])]  
                my_smax_global = smax_global[:, :len(cfg['classnames'])]  

                mcm_global_score = np.max(smax_global, axis=1) # print(mcm_global_score.shape) # [B,]
                my_mcm_global_score = np.max(my_smax_global, axis=1)




                smax_local_tensor = F.softmax(logits_id_local/T, dim=-1)
                # smax_local_tensor_temp = smax_local_tensor.view(batch_size, token_len, cfg['multiply_factor'], len(cfg['classnames']))
                # my_smax_local_tensor = smax_local_tensor_temp[:,:,0,:]


                smax_local = F.softmax(logits_id_local/T, dim=-1).cpu().numpy()
                my_smax_local_tensor = smax_local_tensor[:, :, :len(cfg['classnames'])]
                my_smax_local = smax_local[:, :, :len(cfg['classnames'])]  # [B,196,len(class)]
                

                mcm_local_score = np.max(smax_local, axis=(1, 2))
                # print(logits_id.size)
                my_mcm_local_score = np.max(my_smax_local, axis=(1, 2))
                ...
                id_conf_my_mcm = np.concatenate((id_conf_my_mcm, my_mcm_global_score))
                id_conf_my_glmcm = np.concatenate((id_conf_my_glmcm, my_mcm_global_score + my_mcm_local_score))
                ...
                # caculate accuracy
                pred = my_smax_global_tensor.topk(topk, 1, True, True)[1].t()
                correct = pred.eq(labels.view(1, -1).expand_as(pred))
                acc_num = float(correct[: topk].reshape(-1).float().sum(0, keepdim=True).cpu().numpy())
                correct_samples += acc_num
                all_samples += labels.shape[0]

                logits_id_local_gl = my_smax_local_tensor.max(dim=1)[0] + my_smax_global_tensor
            ...
            avg_auroc_my_mcm, avg_aupr_my_mcm, avg_fpr_my_mcm = 0.0, 0.0, 0.0
            avg_auroc_my_glmcm, avg_aupr_my_glmcm, avg_fpr_my_glmcm = 0.0, 0.0, 0.0
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                ood_conf_my_mcm = np.array([])
                ood_conf_my_glmcm = np.array([])
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    my_smax_global = smax_global[:, :len(cfg['classnames'])]  # [B,len(class)]
                    ...
                    ood_conf_my_mcm = np.concatenate((ood_conf_my_mcm, my_mcm_global_score))
                    ood_conf_my_glmcm = np.concatenate((ood_conf_my_glmcm, my_mcm_global_score + my_mcm_local_score))
                ...
                avg_aupr += aupr
                avg_fpr += fpr
                print(f'AUROC_mcm: {auroc:.4f}, AUPR_mcm: {aupr:.4f}, FPR(0.95)_mcm: {fpr:.4f}')
                ...
                auroc_my_mcm, aupr_my_mcm, fpr_my_mcm = get_measures(id_conf_my_mcm, ood_conf_my_mcm, tpr)
                avg_auroc_my_mcm += auroc_my_mcm
                avg_aupr_my_mcm += aupr_my_mcm
                avg_fpr_my_mcm += fpr_my_mcm
                print(f'AUROC_my_mcm: {auroc_my_mcm:.4f}, AUPR_my_mcm: {aupr_my_mcm:.4f}, FPR(0.95)_my_mcm: {fpr_my_mcm:.4f}')


                auroc_my_glmcm, aupr_my_glmcm, fpr_my_glmcm = get_measures(id_conf_my_glmcm, ood_conf_my_glmcm, tpr)
                avg_auroc_my_glmcm += auroc_my_glmcm
                avg_aupr_my_glmcm += aupr_my_glmcm
                avg_fpr_my_glmcm += fpr_my_glmcm
                print(f'AUROC_my_glmcm: {auroc_my_glmcm:.4f}, AUPR_my_glmcm: {aupr_my_glmcm:.4f}, FPR(0.95)_my_glmcm: {fpr_my_glmcm:.4f}')
            ...
            avg_auroc_my_mcm /= len(ood_dataset_name)
            avg_aupr_my_mcm /= len(ood_dataset_name)
            avg_fpr_my_mcm /= len(ood_dataset_name)
            print(f'Average AUROC_my_mcm: {avg_auroc_my_mcm:.4f}, Average AUPR_my_mcm: {avg_aupr_my_mcm:.4f}, Average FPR(0.95)_my_mcm: {avg_fpr_my_mcm:.4f}')

            avg_auroc_my_glmcm /= len(ood_dataset_name)
            avg_aupr_my_glmcm /= len(ood_dataset_name)
            avg_fpr_my_glmcm /= len(ood_dataset_name)
            print(f'Average AUROC_my_glmcm: {avg_auroc_my_glmcm:.4f}, Average AUPR_my_glmcm: {avg_aupr_my_glmcm:.4f}, Average FPR(0.95)_my_glmcm: {avg_fpr_my_glmcm:.4f}')
