...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_batchs160_ep50','lenx'+str(multiply_factor_name),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            test_file_name = 'log_test_ood_challenging_newscore_forcedplus1origin.txt'
        elif cfg['ood_dataset'] == 'all':
            test_file_name = 'log_test_ood_all.txt'
        else:
            test_file_name = 'log_test_ood_common_newscore_5forcedplus1origin.txt'
        ...
        if ood_dataset_name == 'challenging':
            ood_dataset_name = ['NINCO','OpenImage_O','Imagenet-o' ]
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                # print('smax_global.shape',smax_global.shape) # [B,len(class)*multiply_factor]
                # print(smax_global[1,0:3])
                # print(smax_global[1,1000:1003])
                # print(smax_global[1,2000:2003])
                # smax_global_temp = smax_global.reshape(batch_size, cfg['multiply_factor'], len(cfg['classnames']))
                # my_smax_global = smax_global_temp[:,0,:]
                # my_smax_global = smax_global[:, :len(cfg['classnames'])]  
                my_smax_global_forced = smax_global[:, :len(cfg['classnames'])]  
                my_smax_global_origin = smax_global[:, len(cfg['classnames']):2*len(cfg['classnames'])]
                my_smax_global = 5*my_smax_global_forced + my_smax_global_origin #

                # 


                mcm_global_score = np.max(smax_global, axis=1)
                # print(mcm_global_score.shape) # [B,]
                my_mcm_global_score = np.max(my_smax_global, axis=1)



                smax_local = F.softmax(logits_id_local/T, dim=-1).cpu().numpy()
                # print('smax_local.shape',smax_local.shape) # [B,196,len(class)*multiply_factor]
                # smax_local_temp = smax_local.reshape(batch_size, token_len, cfg['multiply_factor'], len(cfg['classnames']))
                # my_smax_local = smax_local_temp[:,:,0,:]
                # my_smax_local = smax_local[:, :, :len(cfg['classnames'])]  # [B,196,len(class)]
                my_smax_local_forced = smax_local[:, :, :len(cfg['classnames'])]  
                my_smax_local_origin = smax_local[:, :, len(cfg['classnames']):2*len(cfg['classnames'])]  
                my_smax_local = 5*my_smax_local_forced  + my_smax_local_origin  #
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    batch_size, token_len, repeat_len = logits_ood_local.shape
