...
class TextResLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.origin_classnum = int(len(classnames)//cfg['multiply_factor'])
        ...
        if cfg['multiply_factor'] > 1:
            # 其他不学习
            self.text_feature_residuals_multiply = torch.zeros( self.origin_classnum * (cfg['multiply_factor']-1), cfg['embed_dim']).to(self.device)


            # # 共享参数
            # self.text_feature_residuals_multiply = nn.Parameter(torch.zeros( self.origin_classnum, cfg['embed_dim'])).to(self.device)
            # # 将text_feature_residuals_multiply复制cfg['multiply_factor']-1份
            # self.text_feature_residuals_multiply = self.text_feature_residuals_multiply.repeat(cfg['multiply_factor']-1, 1)
            # self.text_feature_residuals = nn.ParameterList([
            #     self.text_feature_residuals_origin,
            #     self.text_feature_residuals_multiply
            # ])

            # # 不共享参数
            # self.text_feature_residuals_multiply = nn.Parameter(torch.zeros( self.origin_classnum * (cfg['multiply_factor']-1), cfg['embed_dim']))
            # self.text_feature_residuals = nn.ParameterList([
            #     self.text_feature_residuals_origin,
            #     self.text_feature_residuals_multiply
            # ])
    ...
    def forward(self, text_features):
        ...
        if self.cfg['multiply_factor'] > 1:
            # # 其他分类头也学习
            # text_feature_residuals = torch.cat(list(self.text_feature_residuals), dim=0)
...
class CustomClipTextEncoder(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_features = get_base_text_features(cfg, classnames, clip_model, self.text_encoder)
    ...
    def forward(self):
        self.text_features = self.text_features.type(self.dtype).to(self.cfg['device'])

        final_text_features = self.text_residual_learner(self.text_features)

        return final_text_features


# class ImgResContrastiveLearner(nn.Module):
#     def __init__(self, cfg, clip_model ):
#         super().__init__()

#         self.contrastive_feature_residuals = nn.Parameter(torch.zeros( 1, cfg['embed_dim'] ))
#         self.embed_dim = cfg['embed_dim']

#         # # 初始化contrastive_feature_residuals参数
#         # nn.init.kaiming_normal_(self.contrastive_feature_residuals, mode='fan_out', nonlinearity='relu')

#     def forward(self ):
#         residual_features = self.contrastive_feature_residuals
...
class CustomClipImageEncoder(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        # self.img_res_contrastive_learner = ImgResContrastiveLearner(cfg, clip_model)
        self.dtype = clip_model.dtype
    ...
    def forward(self, image):
        ...
        final_image_features = image_features   # + self.alpha * self.img_res_contrastive_learner()
...
def main():
    ...
    model_name = 'taskres'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name,'lenx'+str(multiply_factor),'lr00002'  )  #  new_test   seed1
    ...
    if is_train == 1:
        ...
        # cfg['classnames'] = id_dataset.classnames * cfg['multiply_factor']
        ...
        cfg['classnames'] = few_shot_dataset_stage2.classnames * cfg['multiply_factor']
        ...
        print(f"len of mutiply classnames: {len(cfg['classnames'])}")
        ...
        with torch.no_grad():
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                ood_loader = torch.utils.data.DataLoader(ood_dataset, batch_size=cfg['fine_tune_batch_size'],
