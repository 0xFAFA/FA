...
class Skin40():
    ...
    def download_data(self, data_dir, shot):
        ...
        # 构建fewshot数据集
        self.train_zip = list(zip(self.train_data, self.train_targets))
        ...
        for label, items in split_dataset_by_label.items():
            if len(items) >= shot:
                sampled_items = random.sample(items, shot)
            else:
                sampled_items = items
        ...
        print(len(self.train_data_fewshot))
