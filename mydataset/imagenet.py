...
#         self.train = torchvision.datasets.ImageFolder(root=os.path.join(self.image_dir, "train"),transform=train_preprocess)
#         self.val = torchvision.datasets.ImageFolder(root=os.path.join(self.image_dir, "val"), transform=test_preprocess)
...
#         # self.train 对象将具有以下属性： 
#         # samples:
#         # 类型：list
#         # 描述：一个包含所有样本的列表，每个样本是一个元组，包含image path和对应的类别索引

#         # imgs:
#         # 类型：list
#         # 描述：与 samples 相同，包含所有样本的列表，每个样本是一个元组，包含image path和对应的类别索引
...
class CustomImageNet100(DatasetBase):

    dataset_dir = "ImageNet100_MCM"
...
# root_path = '/data/Public/Datasets/'
# test_dataset = CustomImageNet(root=root_path, num_shots=2)

# print(test_dataset.classnames)
