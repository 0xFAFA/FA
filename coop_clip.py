...
class PromptLearner(nn.Module):
    def __init__(self, classnames, clip_model, N_CTX = 16, CTX_INIT = "a photo of a", CSC = False, CLASS_TOKEN_POSITION = "end"):
        ...
        print('tokenized:',tokenized_prompts.shape)
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        print('positional_embedding', self.positional_embedding.shape) #([77, 512])
    ...
    def forward(self, prompts, tokenized_prompts):
        ...
        # print('x.shape', x.shape) # ([len(classname), 512])
        return x
...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name,'imgnet classname lenx1-1 lr0002 ture init'  )  # seed1 imgnet classname lenx7 lr0002
    ...
    if is_train == 1:
        ...
        cfg['classnames'] = few_shot_dataset_stage2.classnames # + few_shot_dataset_stage2.classnames[:100]
        ...
        # Model_stage2
        model_stage2 = CustomCLIP(cfg['classnames'], clip_model ,cfg['template'], False, 16)
        ...
        # ood dataset
        # ood_dataset = OodDataset( cfg['ood_dataset_path'], train_transform_no_aug)
        # ood_loader = torch.utils.data.DataLoader(ood_dataset, batch_size=cfg['fine_tune_batch_size'], shuffle=True, num_workers=8)
