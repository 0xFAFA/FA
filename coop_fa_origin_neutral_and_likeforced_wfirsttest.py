...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        # # type2，先进行split，再进行image_features @ text_features_concat 和 repeat
        # # 即使type1与2在理论上是等价的，但在实际中由于精度问题，还是会有差异，导致train每轮loss不同，最终结果type1稍好
        # text_features_forced = text_features_concat_temp[0, :, :]
        # logits_forced = logit_scale * image_features @ text_features_forced.transpose(-1, -2)
        # logits_local_forced = logit_scale * local_image_features @ text_features_forced.T

        # logits = logits_forced
        # logits_local = logits_local_forced


        # if self.origin_neutral_repeat_num > 0:
        #     text_features_origin_neutral = text_features_concat_temp[1, :, :]

        #     logits_origin_neutral = logit_scale * image_features @ text_features_origin_neutral.transpose(-1, -2)
        #     logits_local_origin_neutral = logit_scale * local_image_features @ text_features_origin_neutral.T

        #     logits_origin_neutral = logits_origin_neutral.repeat(1, self.origin_neutral_repeat_num)
        #     logits_local_origin_neutral = logits_local_origin_neutral.repeat(1, 1, self.origin_neutral_repeat_num)

        #     logits = torch.cat((logits, logits_origin_neutral), dim=1)
        #     logits_local = torch.cat((logits_local, logits_local_origin_neutral), dim=2)


        # if self.origin_likeforced_repeat_num > 0:
        #     if self.origin_neutral_repeat_num > 0:
        #         text_features_origin_likeforced = text_features_concat_temp[2, :, :]
        #     else:
        #         text_features_origin_likeforced = text_features_concat_temp[1, :, :]
        
        #     logits_origin_likeforced = logit_scale * image_features @ text_features_origin_likeforced.transpose(-1, -2)
        #     logits_local_origin_likeforced = logit_scale * local_image_features @ text_features_origin_likeforced.T

        #     logits_origin_likeforced = logits_origin_likeforced.repeat(1, self.origin_likeforced_repeat_num)
        #     logits_local_origin_likeforced = logits_local_origin_likeforced.repeat(1, 1, self.origin_likeforced_repeat_num)

        #     logits = torch.cat((logits, logits_origin_likeforced), dim=1)
        #     logits_local = torch.cat((logits_local, logits_local_origin_likeforced), dim=2)
...
def main():
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'origin_neutral_and_likeforced',multiply_factor_name+'-wfirsttest','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        print(f"origin_neutral_multiply_factor: {origin_neutral_multiply_factor}")
        print(f"origin_likeforced_multiply_factor: {origin_likeforced_multiply_factor}")
        ...
        if cfg['ood_dataset'] == 'challenging':
            test_file_name = 'log_test_ood_challenging_init.txt'
        elif cfg['ood_dataset'] == 'all':
            test_file_name = 'log_test_ood_all_init.txt'
        elif cfg['ood_dataset'] == 'nearood':
            test_file_name = 'log_test_ood_nearood_init.txt'
        elif cfg['ood_dataset'] == 'common':
            test_file_name = 'log_test_ood_common_init.txt'
        else:
