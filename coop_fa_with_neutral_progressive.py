...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, 
        ...
        # register_buffer用于注册不需要梯度更新（即非可学习参数），但需要保存的张量。这些缓冲区会被包含在 state_dict 中，便于next stage中将last stage的text_features_origin_likeforced加载到当前模型中
        # self.register_buffer("text_features_origin_likeforced", text_features_origin_likeforced) 
    ...
    def forward(self, image):
        ...
        return logits, logits_local, text_features_forced
...
def main():
    ...
    model_name = 'fa'
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'with_neutral_progressive',multiply_factor_name+'','lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        # load last stage model
        if cfg['cur_stage'] > 1:
            checkpoint = torch.load(os.path.join(cfg['cache_dir'], f"model_stage{cfg['cur_stage']-1}.pth"))
            # print(checkpoint.keys())
            missing_keys, unexpected_keys = model.load_state_dict(checkpoint, strict=False)
            print("Missing keys:", missing_keys)
            print("Unexpected keys:", unexpected_keys)
            model.module.text_features_origin_likeforced = checkpoint['text_features_origin_likeforced'] # 直接加载上一个stage的text_features_forced为当前stage的text_features_origin_likeforced
        ...
        for train_idx in range(train_epoch):
            ...
            for i, (images, target) in enumerate(tqdm(train_loader)):
                ...
                logits,_,cur_text_features_forced = model(images)
                text_features_forced = cur_text_features_forced
            ...
            if  train_idx == train_epoch - 1 :   
                ...
                with torch.no_grad():
                    ...
                    for images, labels in tqdm(test_loader):
                        ...
                        outputs,_,cur_text_features_forced = model(images)
                        text_features_forced = cur_text_features_forced
                    ...
                    checkpoint['text_features_origin_likeforced'] = text_features_forced # 下一stage的text_features_origin_likeforced为当前stage的text_features_forced
                    torch.save(checkpoint, os.path.join(cfg['cache_dir'], f"model_stage{cfg['cur_stage']}.pth"))
        ...
        # load model
        checkpoint = torch.load(os.path.join(cfg['cache_dir'], f"model_stage{cfg['cur_stage']}.pth"))
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                logits_id, logits_id_local, _ = model(images)
                logits_id /= 100.0
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    logits_ood, logits_ood_local, _ = model(images)
                    logits_ood /= 100.0
