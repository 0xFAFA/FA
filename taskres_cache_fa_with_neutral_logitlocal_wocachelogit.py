...
def main():
    ...
    model_name = 'fa_taskres_cache_check'
    lrname = str(cfg['lr']).replace('.','')
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'with_neutral_trm_train_no_cache_no','neutral_len'+str(origin_neutral_multiply_factor),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        if is_extract_feature == 1:
            ...
            train_loader_extract = build_data_loader(data_source=few_shot_dataset_stage2.train_x, batch_size=train_batch_size, tfm=cache_transform, is_train=True, shuffle=False)
        ...
        if cfg['ood_dataset'] == 'challenging':
            ...
            test_file_name = 'log_test_ood_common_logitlocal_wocachelogit.txt'
        else:
