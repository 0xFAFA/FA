...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        if self.cfg['shots'] != 1:
            keys_res = self.residual_learner_visual.unsqueeze(1).repeat(1, self.cfg['shots']//2, 1).reshape(-1, self.cfg['embed_dim'])
        else:
            keys_res = self.residual_learner_visual
...
def main():
    ...
    model_name = 'fa_taskres_splitcache_entropy_splitres_justhighentropy' #
    lrname = str(cfg['lr']).replace('.','')
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_bs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch']),'trainMtrm_cachenotrm','neutral_len'+str(origin_neutral_multiply_factor),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
