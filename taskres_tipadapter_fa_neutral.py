...
def main():
    ...
    cache_dir = os.path.join('./mycaches_new', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots', model_name, f'ep_taskres{cfg["taskres_train_epoch"]}_tipadapter{cfg["tipadapter_train_epoch"]}', 'trainMtrm_fcacheblurtrm_ocacheblurtrm','neutral_len'+str(origin_neutral_multiply_factor), f'lr_taskres{lrname_taskres}_tipadapter{lrname_tipadapter}','seed'+str(cfg['seed'])  )  #   
    ...
    cache_transform_forced = transform_aug_blur
    ...
    if is_train == 1:
        ...
        # for taskres
        optimizer_taskres = torch.optim.Adam([model_stage2.module.residual_learner_text], lr=cfg['taskres_lr'], weight_decay=5e-4)
        ...
        # for tipadapter
        optimizer_tipadapter = torch.optim.AdamW(model_stage2.module.tip_adapter.parameters(), lr=cfg['tipadapter_lr'], eps=0.001, weight_decay=1e-1) 
