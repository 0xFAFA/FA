...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_batchs160_ep50_zeroshot','lenx'+str(multiply_factor_name),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        for train_idx in range(train_epoch):
            ...
            # test
            if (train_idx + 1) % 100 == 0 or  train_idx == train_epoch - 1 or train_idx == 0  :   #   
                model_stage2.eval()
                with torch.no_grad():
