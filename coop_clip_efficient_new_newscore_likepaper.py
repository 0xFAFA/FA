...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_fa_batchs160_ep30_thenice','lenx'+str(multiply_factor_name),'lr'+lrname,'seed'+str(cfg['seed'])  )  #   
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            test_file_name = 'log_test_ood_challenging_newscore_likepaper.txt'
        elif cfg['ood_dataset'] == 'all':
            test_file_name = 'log_test_ood_all.txt'
        else:
            test_file_name = 'log_test_ood_common_newscore_likepaper.txt'
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                # [B,len(class)] # 以原本的论文公式的方式（即例如mcm得分中分子只看forced prompt部分）
                # 效果不如分子既看forced prompt部分又看origin部分，要解释这个问题首先要关注 origin prompt的作用，
                # origin prompt不仅在训练中用于让模型学到除id类名外的更丰富的特定于id类的语义信息，而且在推理中（此处）origin prompt用于让OOD样本概率分布趋于平均，即让OOD样本感到困惑
