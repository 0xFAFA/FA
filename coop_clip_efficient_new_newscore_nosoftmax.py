...
def main():
    ...
    model_name = 'coop'
    cache_dir = os.path.join('./mycaches_id', cfg['id_dataset'], model_file_name_dict[cfg['backbone']], str(cfg['shots'])+'shots',model_name+'_efficient_batchs'+str(cfg['fine_tune_batch_size'])+'_ep'+str(cfg['fine_tune_train_epoch'])+'','lenx'+str(multiply_factor_name),'lr'+lrname,'seed'+str(cfg['seed'])  )  #    
    ...
    if is_train == 1:
        ...
        if cfg['ood_dataset'] == 'challenging':
            test_file_name = 'log_test_ood_challenging_newscore.txt'
        elif cfg['ood_dataset'] == 'all':
            test_file_name = 'log_test_ood_all.txt'
        else:
            test_file_name = 'log_test_ood_common_newscore_nosoftmax.txt'
        ...
        with torch.no_grad():
            ...
            for images, labels in tqdm(test_loader_id):
                ...
                my_smax_global = logits_id.cpu().numpy()  #[:, :len(cfg['classnames'])]
                # 不取softmax，且只在forced
                ...
                my_smax_local = logits_id_local.cpu().numpy()  # [:, :, :len(cfg['classnames'])]
            ...
            for cur_ood_dataset in ood_dataset_name:
                ...
                for images, _ in tqdm(ood_loader):
                    ...
                    # smax_global_temp = smax_global.reshape(batch_size, cfg['multiply_factor'], len(cfg['classnames']))
                    # my_smax_global = smax_global_temp[:,0,:]
                    my_smax_global = logits_ood.cpu().numpy()  # [:, :len(cfg['classnames'])]
                    ...
                    # smax_local_temp = smax_local.reshape(batch_size, token_len, cfg['multiply_factor'], len(cfg['classnames']))
                    # my_smax_local = smax_local_temp[:,:,0,:]
                    my_smax_local = logits_ood_local.cpu().numpy()  # [:, :, :len(cfg['classnames'])]
